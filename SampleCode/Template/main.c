...
volatile uint32_t previous_LIRC_counter = 0;
volatile uint32_t current_LIRC_counter = 0;
volatile uint32_t tarcurrent_LIRC_counter = 0;

volatile uint8_t calibrate_counter = 0;
...
{	
    if(TIMER_GetIntFlag(TIMER2) == 1)
    {
        ...
        current_LIRC_counter = TIMER_GetCounter(TIMER1);       
    }
}
...
void TIMER1_Init(void)
{
    ...
    #else  // after calibrate
    TIMER2_Init();

    TIMER_ResetCounter(TIMER1);
    TIMER1->CTL |= TIMER_PERIODIC_MODE ;
    TIMER1->INTSTS = (TIMER_INTSTS_TWKF_Msk | TIMER_INTSTS_TIF_Msk);  /* clear interrupt status */
    ...
    TIMER_Stop(TIMER0);
    TIMER_ResetCounter(TIMER1);
    TIMER1->CMP = current_LIRC_counter;
